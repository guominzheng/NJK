...
<script type="text/javascript">
    ...
    //删除图片
    function delP(IMG,FID){
        if(confirm("确定要删除图片？")){
            var url = "post_special/deltp.do?IMG="+IMG+"&FID="+FID+"&guid="+new Date().getTime();
    ...
    //删除图片
    function delPP(COVER_IMG,FID){
        if(confirm("确定要删除图片？")){
            var url = "post_special/deltpp.do?COVER_IMG="+COVER_IMG+"&FID="+FID+"&guid="+new Date().getTime();
            $.get(url,function(data){
                    document.location.reload();
                if(data=="success"){
                    $("#uploadName2").val("");
    ...
    //删除图片
    function delPPP(COVER_IMG,FID){
        if(confirm("确定要删除图片？")){
            var url = "post_special/deltppp.do?COVER_IMG2="+COVER_IMG+"&FID="+FID+"&guid="+new Date().getTime();
            $.get(url,function(data){
                    document.location.reload();
                if(data=="success"){
                    $("#uploadName3").val("");
...
<body>
    <form action="post_special/${msg }.do" name="Form" id="Form" method="post">
        ...
        <input type="hidden" name="VIEWS" id="VIEWS" value="${pd.VIEWS}"/>
        ...
        <table id="table_report" class="table table-striped table-bordered table-hover">
            <tr>
                ...
                <td><input type="text" name="SPECIAL" id="SPECIAL" value="${pd.SPECIAL}" maxlength="32" placeholder="这里输入专题名称" title="专题名称"/></td>
            </tr>
            <tr>
                <td style="width:70px;text-align: right;padding-top: 13px;">作者名称:</td>
                <td><input type="text" name="NAME" id="NAME" value="${pd.NAME}" maxlength="32" placeholder="这里输入作者名称" title="作者名称"/></td>
            </tr>
            <tr>
                <td style="width:70px;text-align: right;padding-top: 13px;">专题简介:</td>
                <td><textarea cols="30" rows="3" align="center" name="MESSAGE" id="MESSAGE">${pd.MESSAGE}</textarea></td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td>
                    ...
                    <c:if test="${(not empty pd.IMG) && (pd.IMG ne '')}">
                        <div id="delP">
                            <input type="button"  class="btn btn-mini btn-danger"  value="删除图片" onclick="delP('${pd.IMG}','${pd.FID }');" /><br>
              ...
              <tr>
                <td style="width:70px;text-align: right;padding-top: 13px;">封面图片:<input type="hidden" id="uploadName2" name="COVER_IMG" value="${pd.COVER_IMG}"/></td>
                <td>
                    <c:if test="${(empty pd.COVER_IMG) || (pd.COVER_IMG eq '')}">
                    ...
                    <c:if test="${(not empty pd.COVER_IMG) && (pd.COVER_IMG ne '')}">
                        <div id="delPP">
                            <input type="button"  class="btn btn-mini btn-danger"  value="删除图片" onclick="delPP('${pd.COVER_IMG}','${pd.FID }');" /><br>
                            <a href="${pd.COVER_IMG}" target="_blank"><img src="${pd.COVER_IMG}" width="100"/></a>
                        </div>
                    </c:if>
                </td>
            </tr>
               <tr>
                <td style="width:70px;text-align: right;padding-top: 13px;">封面图片:<input type="hidden" id="uploadName3" name="COVER_IMG2" value="${pd.COVER_IMG2}"/></td>
                <td>
                    <c:if test="${(empty pd.COVER_IMG2) || (pd.COVER_IMG2 eq '')}">
                    ...
                    <c:if test="${(not empty pd.COVER_IMG2) && (pd.COVER_IMG2 ne '')}">
                        <div id="delPPP">
                            <input type="button"  class="btn btn-mini btn-danger"  value="删除图片" onclick="delPPP('${pd.COVER_IMG2}','${pd.FID }');" /><br>
                            <a href="${pd.COVER_IMG2}" target="_blank"><img src="${pd.COVER_IMG2}" width="100"/></a>
                        </div>
                    </c:if>
                </td>
            </tr>
