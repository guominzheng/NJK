...
<script type="text/javascript">
	...
	//删除图片
	function delP(IMG,WENDA_TIEZI_ID){
		if(confirm("确定要删除图片？")){
			var url = "wendaTieZi/deltp.do?IMG="+IMG+"&WENDA_TIEZI_ID="+WENDA_TIEZI_ID+"&guid="+new Date().getTime();
	...
	function delPP(IMG,WENDA_TIEZI_ID){
		if(confirm("确定要删除图片？")){
			var url = "wendaTieZi/deltpp.do?FIMG="+IMG+"&WENDA_TIEZI_ID="+WENDA_TIEZI_ID+"&guid="+new Date().getTime();
			$.get(url,function(data){
					document.location.reload();
				if(data=="success"){
					$("#uploadName").val("");
...
<body>
	<form action="wendaTieZi/${msg }.do" name="Form" id="Form" method="post">
		<input type="hidden" name="WENDA_TIEZI_ID" id="WENDA_TIEZI_ID" value="${pd.WENDA_TIEZI_ID}"/>
		...
		<table id="table_report" class="table table-striped table-bordered table-hover">
			...
			<tr>
				<td style="width:120px;text-align: right;padding-top: 13px;">分享简介:</td>
				<td><textarea name="JIANJIE" id="JIANJIE" style="width:200px;height:80px;" placeholder="这里输入分享简介" title="分享简介">${pd.JIANJIE}</textarea>
				<%-- <input type="image" name="WENDA_SHIJUAN_MIAOSHU" id="WENDA_SHIJUAN_MIAOSHU" value="${pd.WENDA_SHIJUAN_MIAOSHU}" placeholder="这里输入试卷描述" title="试卷描述"/> --%></td>
			</tr>
			<tr>
				<td style="width:70px;text-align: right;padding-top: 13px;">排序:</td>
				<td><input style="width:300PX" type="text" name="ORDE_BY" id="ORDE_BY" value="${pd.ORDE_BY}" maxlength="300" placeholder="排序" title="排序"/></td>
			</tr>
			<tr>
			...
			<select class="chzn-select" name="STATUS" id="STATUS"  data-placeholder="请选择类型" title="类型" style="vertical-align: top; width: 145px;" onchange="jz();">
                     ...
                     <option value="4" <c:if test="${pd.STATUS=='4' }">selected</c:if>>请选择答题</option>
                     <option value="5" <c:if test="${pd.STATUS=='5' }">selected</c:if>>广告置顶</option>
            </select></td>
			</tr>
			<tr>
				<td style="width:70px;text-align: right;padding-top: 13px;">二级分类:</td>
				<td style="vertical-align:top;">
					<select name="ID" id="ID"  style="vertical-align:top;width: 120px;">
					<option value="">请选择分类</option>
					<c:forEach items="${varList}" var="var" varStatus="vs">
						<c:if test="${pd.STATUS=='0'}">
							<option value="${var.TID}" <c:if test="${var.TID == pd.ID }">selected</c:if>>${var.SUBJECT}</option>
						</c:if>
						<c:if test="${pd.STATUS=='1'}">
							<option value="${var.PRODUCT_ID}" <c:if test="${var.PRODUCT_ID == pd.ID }">selected</c:if>>${var.PRODUCT_NAME}</option>
						</c:if>
						<c:if test="${pd.STATUS=='2'}">
							<option value="${var.FID}" <c:if test="${var.FID == pd.ID }">selected</c:if>>${var.SPECIAL}</option>
						</c:if>
						<c:if test="${pd.STATUS=='3'}">
							<option value="${var.ACTIVITY_ID}" <c:if test="${var.ACTIVITY_ID == pd.ID }">selected</c:if>>${var.TITLE}</option>
						</c:if>
						<c:if test="${pd.STATUS=='4'}">
							<option value="${var.WENDA_SHIJUAN_ID}" <c:if test="${var.WENDA_SHIJUAN_ID == pd.ID }">selected</c:if>>${var.WENDA_SHIJUAN_NAME}</option>
						</c:if>
						<c:if test="${pd.STATUS=='5'}">
							<option value="5">跳转网页</option>
						</c:if>
					</c:forEach>
					</select>
				</td>
			...
			<tr>
                ...
                <td><input type="text" name="URL" id="URL"  value="${pd.URL}" maxlength="32" placeholder="这里输入url(广告的时候需要url)" title="URL"/></td>
        	</tr>
			<tr>
				...
				<td>
					...
					<c:if test="${(not empty pd.IMG) && (pd.IMG ne '')}">
						<div id="delP">
							<input type="button"  class="btn btn-mini btn-danger"  value="删除图片" onclick="delP('${pd.IMG}','${pd.WENDA_TIEZI_ID}');" /><br>
			...
			<tr>
                <td style="width:70px;text-align: right;padding-top: 13px;">分享图:<input type="hidden" id="uploadName2" name="FIMG" value="${pd.FIMG}"/></td>
                <td>
                    <c:if test="${(empty pd.FIMG) || (pd.FIMG eq '')}">
                    ...
                    <c:if test="${(not empty pd.FIMG) && (pd.FIMG ne '')}">
                        <div id="delPP">
                            <input type="button"  class="btn btn-mini btn-danger"  value="删除图片" onclick="delPP('${pd.FIMG}','${pd.WENDA_TIEZI_ID }');" /><br>
                            <a href="${pd.FIMG}" target="_blank"><img src="${pd.FIMG}" width="100"/></a>
                        </div>
                    </c:if>
                </td>
            </tr>
			 ...
			 var url = "<%=basePath%>banner/findList.do?STATUS="+$("#STATUS").val()+"&tm="+new Date().getTime();
				$.get(url,function(varList){
                   ...
                   $.each(eval(varList), function (i, item) {
                   	if(item.TID!=null){
                   		$("<option value='" + item.TID + "'>" + item.SUBJECT + "</option>").appendTo($("#ID"));
                   	}else if(item.PRODUCT_ID!=null){
                   		$("<option value='" + item.PRODUCT_ID + "'>" + item.PRODUCT_NAME + "</option>").appendTo($("#ID"));
                   	}else if(item.FID!=null&&item.FID!=0&&item.TID==null){
                   		$("<option value='" + item.FID + "'>" + item.SPECIAL + "</option>").appendTo($("#ID"));
                   	}else if(item.ACTIVITY_ID!=null){
                   		$("<option value='" + item.ACTIVITY_ID + "'>" + item.TITLE + "</option>").appendTo($("#ID"));
                   	}else if(item.WENDA_SHIJUAN_ID!=null){
                   		$("<option value='" + item.WENDA_SHIJUAN_ID + "'>" + item.WENDA_SHIJUAN_NAME + "</option>").appendTo($("#ID"));
                   	}
                   });
               }
				});
