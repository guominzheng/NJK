                                ...
                                <c:choose>
                                            ...
                                            <c:forEach items="${varList}" var="var" varStatus="vs">
                                                <tr>
                                                    <td class='center' style="width: 30px;"><label><input
                                                            type='checkbox' name='ids' value="${var.CUSTOMER_ID}" /><span
                                                            class="lbl"></span></label></td>
                                                    <td class='center' style="width: 30px;">${vs.index+1}</td>
                                                    <td class='center'>${var.CUSTOMER_NAME}</td>
                                                    <td class='center'>${var.PHONE}</td>
                                                    <td class='center'>${var.BPHONE}</td>
                                                    <td class='center'>${var.CROP}</td>
                                                    <td class='center'>${var.P_NAME} ${var.C_NAME} ${var.D_NAME} ${var.ADDRESS}</td>
                                                    <td class='center'>${var.BEIZHU}</td>
                                                    <td class='center'>${var.WEIXIN}</td>
                                                  
                                                    <td class='center'>${var.QQ}</td>
                                                    <td class='center'>${var.REGISTER_DATE}</td>
                                                    <td class='center'>${var.REGISTER_NAME}</td>
                                    ...
                                    <div class='hidden-phone visible-desktop btn-group'>
                                        ...
                                        <ul class="dropdown-menu dropdown-icon-only dropdown-light pull-right dropdown-caret dropdown-close">
                                            <c:if test="${QX.edit == 1 }">
                                            <li><a style="cursor:pointer;" title="编辑" onclick="edit('${var.CUSTOMER_ID}');" class="tooltip-success" data-rel="tooltip" title="" data-placement="left"><span class="green"><i class="icon-edit"></i></span></a></li>
                                            </c:if>
                                            <c:if test="${QX.del == 1 }">
                                            <li><a style="cursor:pointer;" title="删除" onclick="del('${var.CUSTOMER_ID}');" class="tooltip-error" data-rel="tooltip" title="" data-placement="left"><span class="red"><i class="icon-trash"></i></span> </a></li>
                                            </c:if>
                                        </ul>
                                        </div>
                                    </div>
