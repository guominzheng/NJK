...
<script type="text/javascript">
    ...
    //删除图片
    function delP(IMG,ACTIVITY_ID,USER_ID){
        if(confirm("确定要删除图片？")){
            var url = "activity_post/deltp.do?IMG="+IMG+"&ACTIVITY_ID="+ACTIVITY_ID+"&USER_ID="+USER_ID;
...
<body>
    <form action="activity_post/${msg }.do" name="Form" id="Form" method="post">
    	<input type="hidden" name="ACTIVITY_ROOM_ID" id="ACTIVITY_ROOM_ID" value="${pd.ACTIVITY_ROOM_ID}"/>
        <input type="hidden" name="ACTIVITY_ID" id="ACTIVITY_ID" value="${pd.ACTIVITY_ID}"/>
        <input type="hidden" name="USER_ID" id="USER_ID" value="${pd.USER_ID}"/>
        <div id="zhongxin">
        <table id="table_report" class="table table-striped table-bordered table-hover">
            <tr>
                <td style="width:70px;text-align: right;padding-top: 13px;">作物:</td>
                <td><input type="text" name="CROP" id="CROP" value="${pd.CROP}" maxlength="32" placeholder="这里输入作物" title="作物"/></td>
            </tr>
            <tr>
                ...
                <td>
                    ...
                    <c:if test="${(not empty pd.IMG) && (pd.IMG ne '')}">
                        <div id="delP">
                            <input type="button"  class="btn btn-mini btn-danger"  value="删除图片" onclick="delP('${pd.IMG}','${pd.ACTIVITY_ID }','${pd.USER_ID}');" /><br>
