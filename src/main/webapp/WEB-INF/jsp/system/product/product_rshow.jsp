											...
											<c:forEach items="${varList}" var="var" varStatus="vs">
												<tr>
													...
													<td class='center'><span
														class="label label-success arrowed">${var.SUBJECT}</span></td>
													...
													<td class='center'>
														<c:if test="${var.LSTATUS==0}">
															<a class="btn btn-small btn-success"
															   onclick="updateL('${var.RESEARCH_ID}');">暂无</a>
														</c:if>
														<c:if test="${var.LSTATUS==1}">
															<a class="btn btn-small btn-success"
															   onclick="updateL('${var.RESEARCH_ID}');">商品</a>
														</c:if>
														<c:if test="${var.LSTATUS==2}">
															<a class="btn btn-small btn-success"
																	onclick="updateL('${var.RESEARCH_ID}');">版本</a>
														</c:if>
														</td>
                                                    <td class='center'><a
														class="btn btn-small btn-success"
														onclick="fabu('${var.RESEARCH_ID}');">发布</a></td>
													<td class='center'><a
															class="btn btn-small btn-success"
															onclick="lookComment('${var.RESEARCH_ID}');">查看评论</a></td>
													<td id="checkRadio" class='center'>
														<label style="float:left;padding-left: 12px;"><input name="form-field-radio${vs.index+1}" id="form-field-radio${vs.index+1}" onclick="setType('0','${var.RESEARCH_ID}');" <c:if test="${var.TSTATUS == '0' }">checked="checked"</c:if> type="radio" value="icon-edit"><span class="lbl">否</span></label>
														<label style="float:left;padding-left: 5px;"><input name="form-field-radio${vs.index+1}" id="form-field-radio${vs.index+1}" onclick="setType('1','${var.RESEARCH_ID}');" <c:if test="${var.TSTATUS == '1' }">checked="checked"</c:if> type="radio" value="icon-edit"><span class="lbl">是</span></label>
													</td>
							...
							<table style="width: 100%;">
								<tr>
									<td style="vertical-align: top;"><c:if
											test="${QX.add == 1 }">
											<a class="btn btn-small btn-success" onclick="add('${pd.PRODUCT_ID}');">添加研究院</a>
										</c:if> 
										<%-- <c:if test="${QX.del == 1 }">
	...
	<script type="text/javascript">
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>product/rdelete.do?RESEARCH_ID="+Id+"&tm="+new Date().getTime();
					$.get(url,function(data){
						nextPage("${page.currentPage}");
					});
				}
		...
		function add(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>product/goRAdd.do?PRODUCT_ID='+id;
			 diag.Width = 1000;
			 diag.Height = 1000;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
        ...
        function lookComment(id){
            ...
            diag.Drag=true;
            diag.Title ="评论查看";
            diag.URL = '<%=basePath%>product/goLookRes.do?RESEARCH_ID='+id;
            diag.Width = 1000;
            diag.Height = 1000;
            diag.CancelEvent = function(){ //关闭事件
                if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
        ...
        function updateL(id){
            ...
            diag.Drag=true;
            diag.Title ="评论查看";
            diag.URL = '<%=basePath%>product/updateLgo.do?RESEARCH_ID='+id;
            diag.Width = 1000;
            diag.Height = 1000;
            diag.CancelEvent = function(){ //关闭事件
                if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
        ...
        function fabu(Id){
            bootbox.confirm("确定要发布吗?", function(result) {
                if(result) {
                    top.jzts();
                    var url = "<%=basePath%>product/fabu.do?RESEARCH_ID="+Id+"&tm="+new Date().getTime();
                    $.get(url,function(data){
                        nextPage("${page.currentPage}");
                    });
                }
