    ...
    <div id="page-content" class="clearfix">
        ...
        <div class="row-fluid">
            ...
            <div class="row-fluid">
                ...
                <!-- 检索  -->
                <form action="comment_opin/list" method="post" name="Form" id="Form">
                    <input type="hidden" id="OPINION_ID" name="OPINION_ID" value="${pd.OPINION_ID}"/>
                    <table>
                        <tr>
                            ...
                            <td style="vertical-align: top;"><select
                                    ...
                                    data-placeholder="请选择活动使用情况" title="优惠活动"
                                    style="vertical-align: top; width: 145px;">
                                <option value="">请选择活动使用情况</option>
                                <option value="0"
                                >否</option>
                                <option value="1"
                                >是</option>
                                    ...
                                    <c:forEach items="${varList}" var="var" varStatus="vs">
                                        <tr>
                                            <td class='center' style="width: 30px;"><label><input
                                                    type='checkbox' name='ids' value="${var.COMMENT_OPIN_ID}"/><span
                                                    class="lbl"></span></label></td>
                                            <td class='center' style="width: 30px;">${vs.index+1}</td>
                                            <td class='center' width="100px"><span
                                                    class="label label-success arrowed">${var.USERNAME}</span></td>
                        ...
                        <table style="width: 100%;">
                            <tr>
                                <td style="vertical-align: top;">
                                    <c:if
                                            test="${QX.add == 1 }">
                                        <a class="btn btn-small btn-success" onclick="add('${pd.OPINION_ID}','${pd.USER_ID}');">回复</a>
                                    </c:if>
                                     <c:if test="${QX.del == 1 }">
...
<script type="text/javascript">
    ...
    function add(id,uid) {
        ...
        diag.Drag = true;
        diag.Title = "新增";
        diag.URL = '<%=basePath%>comment_opin/gosave.do?OPINION_ID='+id+'&USER_ID='+uid;
        diag.Width = 700;
        diag.Height = 700;
        ...
        bootbox.confirm(msg, function (result) {
            if (result) {
                ...
                if (str == '') {
                    ...
                    if (msg == '确定要删除选中的数据吗?') {
                        top.jzts();
                        var opn = $("#OPINION_ID").val();
                        $.ajax({
                            type: "POST",
                            url: '<%=basePath%>comment_opin/deleteAll?OPINION_ID='+opn+'&tm=' + new Date().getTime(),
                            data: {DATA_IDS: str},
                            dataType: 'json',
