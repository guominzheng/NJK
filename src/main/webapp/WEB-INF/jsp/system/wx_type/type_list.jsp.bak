...
<div class="container-fluid" id="main-container">
    ...
    <div id="page-content" class="clearfix">
        ...
        <div class="row-fluid">
            ...
            <div class="row-fluid">
                <!-- 检索  -->
                <form action="wxController/typeList" method="post" name="Form" id="Form">
                    <table>
                    </table>
                    <!-- 检索  -->
                            ...
                            <c:when test="${not empty varList}">
                                <c:if test="${QX.cha == 1 }">
                                    <c:forEach items="${varList}" var="var" varStatus="vs">
                                        <tr>
                                            <td class='center' style="width: 30px;"><label><input type='checkbox' name='ids' value="${var.type_id}"/><span class="lbl"></span></label></td>
...
<script type="text/javascript">
    ...
    function add() {
        ...
        diag.Drag = true;
        diag.Title = "新增";
        diag.URL = '//hitman.net.cn/NJK/wxController/typeSave.do';
        diag.Width = 300;
        diag.Height = 200;
    ...
    function del(Id) {
        bootbox.confirm("确定要删除吗?", function (result) {
            if (result) {
                top.jzts();
                var url = "//hitman.net.cn/NJK/wxController/typeDelete?type_id=" + Id + "&tm=" + new Date().getTime();
    ...
    function edit(Id) {
        ...
        diag.Drag = true;
        diag.Title = "编辑";
        diag.URL = '//hitman.net.cn/NJK/wxController/typeEdit.do?type_id=' + Id;
        diag.Width = 300;
        diag.Height = 200;
        diag.CancelEvent = function () { //关闭事件
            if (diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none') {
...
<script type="text/javascript">
            ...
            if (result) {
                ...
                if (str == '') {
                    ...
                    if (msg == '确定要删除选中的数据吗?') {
                        ...
                        $.ajax({
                            type: "POST",
                            url: '//hitman.net.cn/NJK/wxController/typeDeleteAll.do?tm=' + new Date().getTime(),
                            data: {DATA_IDS: str},
                            dataType: 'json',
    ...
    function show(id){
        ...
        diag.Drag=true;
        diag.Title ="上传音频";
        diag.URL = '//hitman.net.cn/NJK/wxController/saveAudio?type_id='+id;
        diag.Width = 500;
        diag.Height = 800;
        diag.CancelEvent = function(){ //关闭事件
            if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
