...
/*	@RequestMapping(value = "/map2")
	public ModelAndView map2() throws Exception {
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		List<PageData> list=expressuserService.findList(pd);
		String object="[";
		for(int i=0;i<list.size();i++){
			if(list.get(i).getString("LNG")!=null&&list.get(i).getString("LAT")!=null&&list.get(i).getString("PHONE")!=null&&list.get(i).getString("NAME")!=null){
				String NAME="\""+list.get(i).getString("NAME")+""+list.get(i).getString("PHONE")+""+"\"";
				if(list.size()-i==1){
					object+="["+list.get(i).getString("LNG")+","+list.get(i).getString("LAT")+","+NAME+"]";	
				}else{
					object+="["+list.get(i).getString("LNG")+","+list.get(i).getString("LAT")+","+NAME+"]"+",";
				}
			}
		...
		object+="]";
		pd.put("object", object);
		System.out.println(object);
		mv.setViewName("system/tools/map2");
		mv.addObject("pd", pd);
		return mv;
	}*/
